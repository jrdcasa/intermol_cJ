...
class GromacsParser(object):
    ...
    def canonical_dihedral(self, params, dihedral, direction='into'):
        ...
        if direction == 'into':
            if dihedral == ProperPeriodicDihedralType:
                ...
                convertfunc = convert_dihedral_from_fourier_to_trig
                converted_dihedral = TrigDihedral
            # cJ multiple proper dihedral
            elif dihedral == ProperMultipleDihedral:
    ...
    def choose_parameter_kwds_from_forces(self, entries, n_atoms, force_type,
        ...
        kwds = dict()
        if typename == "ProperMultipleDihedralType":
            m = entries[7]
            m = 0
            for entry in entries[5:]:
                label = 'k'+'{0:1s}'.format(str(m))
                kwds[label] = float(entry) * u[m]
                m += 1
        else:
    ...
    def create_dihedral(self, dihedral):
        ...
        dihedral_types = [None]
        # cJ Multiple propers
        if numeric_dihedraltype == '9':
            gromacs_dihedral = self.gromacs_dihedrals[numeric_dihedraltype]
            dihedral_types = ["ProperMultipleDihedral"]
        # cJ Multiple propers - end
            ...
            # cJ: Remove 9
            if numeric_dihedraltype in ['1', '3', '4', '5']:
    ...
    def process_file(self, top_filename):
        ...
        # cJ Modify the self.molecule_types[imol].dihedrals
        # cJ Lookup multiple proper dihedrals and group all in the same angle
        dieh_toxwaerd = defaultdict()
        new_dihedrals = list()
        for imol in self.molecule_types:
            for idieh in self.molecule_types[imol].dihedrals:
                if len(idieh) == 8:
                    # Just for dihedrals of type 9 in GROMACS
                    # (https://manual.gromacs.org/documentation/2019.3/reference-manual/topologies/topology-file-formats.html)
                    if idieh[4] == '9':
                        key = idieh[0]+"-"+idieh[1]+"-"+idieh[2]+"-"+idieh[3]+"-9"
                        if key in dieh_toxwaerd.keys():
                            dieh_toxwaerd[key].append(idieh[6])
                        else:
                            dieh_toxwaerd[key] = [idieh[6]]
                else:
                    new_dihedrals.append(idieh)
            # Type Diehdrals different to Toxvaerd
            self.molecule_types[imol].dihedrals = new_dihedrals
            #
            # Toxvaerd potentials
            for key, values in dieh_toxwaerd.items():
                atom_list = key.split("-")
                self.molecule_types[imol].dihedrals.append([*atom_list, *values])
